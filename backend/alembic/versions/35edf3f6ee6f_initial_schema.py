...
def upgrade() -> None:
    ...
    op.create_index(
        op.f("ix_snomed_concepts_concept_id"),
        "snomed_concepts",
        ["concept_id"],
        unique=False,
    )
    ...
    op.create_index(
        op.f("ix_snomed_descriptions_concept_id"),
        "snomed_descriptions",
        ["concept_id"],
        unique=False,
    )
    op.create_index(
        op.f("ix_snomed_descriptions_id"), "snomed_descriptions", ["id"], unique=False
    )

    # GIN trgm index
    op.execute(
        "CREATE INDEX IF NOT EXISTS trgm_idx_snomed_term ON snomed_descriptions USING gin (term gin_trgm_ops);"
    )
...
def downgrade() -> None:
    ...
    op.drop_index(
        op.f("ix_snomed_descriptions_concept_id"), table_name="snomed_descriptions"
    )
